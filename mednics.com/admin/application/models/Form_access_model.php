...
class Form_access_model extends CI_Model 
{
	...
	*/
	...
	public function persondata($usr_id='')
	{
    	...
    	$sql1="select  * FROM `adm_person` where prs_id=".$usr_id."";
	...
	function display_access($usr_id)
	{
		...
		foreach($by_combo->result() as $mod)
		{
			...
			$this->db->from('adm_sub_menu_master');
	...
	function change_access()
	{
		...
		for($i=0;$i<$marlen-1;$i++)
		{
			...
			if($inar[0]==0)
			{
				...
				if($inar[1]==1)
				{
					$sq="insert into adm_form_access(`fma_USR_id`,`fma_mnu_id`,`fma_sbm_id`,`fma_access`,`fma_read`,`fma_update`,`fma_delete`,`fma_write`) value ('".$inar[6]."','".$inar[7]."','".$inar[8]."','".$inar[1]."','".$inar[2]."','".$inar[3]."','".$inar[4]."','".$inar[5]."')";
					$this->db->query($sq);
				}
				else
				{
			...
			{
				//echo "UPDATE";
				$sq="update adm_form_access set fma_access='".$inar[1]."',`fma_read`='".$inar[2]."',`fma_update`='".$inar[3]."',`fma_delete`='".$inar[4]."',`fma_write`='".$inar[5]."' where fma_id ='".$inar[0]."'";
				//echo $sq;
				$this->db->query($sq);
				
				if($inar[1]=="1")
				{
					$sqy="update adm_form_access set fma_Status='Y' where fma_id ='".$inar[0]."' ";
					$this->db->query($sqy);
				}				
			}
			}
