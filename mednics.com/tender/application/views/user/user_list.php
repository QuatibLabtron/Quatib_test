...
<html lang="en">
  ...
  <body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white">
              ...
              <div class="col-md-12 ">
                <div class="portlet light bordered">
                  <div class="portlet-title">
                    <span class="caption-subject bold uppercase font-orange">User list </span>
                    <a href="<?php echo site_url('add-user')?>" class="btn btn green"><i class="fa fa-plus"></i> Add User</a> 
                  ...
                  <div class="portlet-body">
                    <form method="post" enctype="multipart/form-data" id="frmListing">
                      <table class="table table-striped table-bordered table-hover user-list" id="sample_1">
                          <thead>
                              <tr>
                                  <th> </th>
                                  <th>Name</th>
    ...
    <script type="text/javascript">
      $(document).ready(function(){getUserlist();});

      function getUserlist()
      {
        var customDataTableElement = '.user-list';
        $(customDataTableElement).DataTable().destroy();
        var customDataTableCount   = '<?php echo $dataTableData->table_data_count; ?>';
        var customDataTableServer  = <?php echo $dataTableData->table_server_status; ?>;
        var customDataTableUrl     =  baseUrl + 'getuserlist?table_data_count='+customDataTableCount;
        var customDataTableColumns = [
          ...
          { 'data'  : 'prs_name' ,
            'render': function(data, type, row, meta)
            {
                link = ` <a href="`+baseUrl+`user-details-`+row.prs_username+`" title=" Detail View "> `+row.prs_name+`</a> `;
                return link;
            }
          },
          {   'data'  : 'prs_email' },
      ...
      function ConverToActive()
      {
        ...
        if (!checked){ alert("Please select at least one record to Activate"); return false; }
        if(!confirm('Do you really want to Activate this entries')) { return; }
        ...
        $.ajax(
        {
          ...
          success: function(response)
          {
            ...
            {
              swal({ title: "Done", text: response.message, type: "success", icon: "success",  button: true }).then(()=>{  getUserlist(); });
            }
            else
            {
              swal({ title: "Opps", text: "Something Went wrong", type: "error", icon: "error", button: true });
            }
          }
        });
      }
      ...
      {
        var checked = jQuery('input[name="chkId[]"]:checked').length > 0;
        if (!checked){ alert("Please select at least one record to Deactive"); return false; }
        if(!confirm('Do you really want to Deactive this entries')) { return; }
        var dataString = $('#frmListing').serialize();
        $.ajax(
        {
          type: "POST",
          url: baseUrl + "user-deactivate",
          data : dataString,
          success: function(response)
          {
            response = JSON.parse(response);
